...
class BartTextCNN(Module):
    ...
    def __init__(
        ...
        # Bart
        self.config = AutoConfig.from_pretrained(model_name_or_path, cache_dir=cache_dir, num_labels=num_labels)
        self.encoder = AutoModel.from_pretrained(model_name_or_path, config=self.config)

        # TextCNN
        self.conv = torch.nn.ModuleList([torch.nn.Conv2d(1, num_filters, (K, self.config.d_model)) for K in filter_sizes])
        self.classifier = torch.nn.Linear(len(filter_sizes) * num_filters, self.config.num_labels)
        ...
        self.dense = Linear(feature_size, second_hidden_size)
        self.classifier = Linear(second_hidden_size, num_labels)
    ...
    def forward(
        ...
        pooled_output = hidden_states.unsqueeze(1)
        pooled_output = [F.relu(conv(pooled_output)).squeeze(3) for conv in self.conv]
        pooled_output = [F.max_pool1d(i, i.size(2)).squeeze(2) for i in pooled_output]
        pooled_output = torch.cat(pooled_output, 1)
        pooled_output = self.dropout(pooled_output)
...
class Bart(Module):
    ...
    def __init__(
        ...
        if feature_size > 0:
            self.feature_projection = Linear(feature_size, feature_proj_size)
        else:
            self.feature_projection = None
        self.dense = Linear(self.config.d_model+feature_proj_size, self.config.d_model+feature_proj_size)
        self.classifier = Linear(self.config.d_model+feature_proj_size, num_labels)

        if multi_task:
            self.value_classifier = Linear(self.config.d_model+feature_proj_size, num_rating)
            self.location_classifier = Linear(self.config.d_model+feature_proj_size, num_rating)
            self.rooms_classifier = Linear(self.config.d_model+feature_proj_size, num_rating)
            self.service_classifier = Linear(self.config.d_model+feature_proj_size, num_rating)
            self.sleep_classifier = Linear(self.config.d_model+feature_proj_size, num_rating)
            self.cleanliness_classifier = Linear(self.config.d_model+feature_proj_size, num_rating)
    ...
    def forward(
        ...
        feature_representation = torch.tensor([], dtype=torch.float32, device=sentence_representation.device)
        if self.add_stay_date:
            feature_representation = torch.cat([feature_representation, stay_year, stay_month], axis=-1)
        if self.add_rating:
            feature_representation = torch.cat([feature_representation, rating], axis=-1)
        if self.add_pandemic:
            feature_representation = torch.cat([feature_representation, pandemic], axis=-1)
        if self.feature_projection is not None:
            feature_representation = self.feature_projection(feature_representation)
            sentence_representation = torch.cat([sentence_representation, feature_representation], axis=-1)

        sentence_representation = self.dropout(sentence_representation)
        sentence_representation = self.dense(sentence_representation)
        ...
        if self.multi_task:
            value_logits = self.value_classifier(sentence_representation)
            location_logits = self.location_classifier(sentence_representation)
            rooms_logits = self.rooms_classifier(sentence_representation)
            service_logits = self.service_classifier(sentence_representation)
            sleep_logits = self.sleep_classifier(sentence_representation)
            cleanliness_logits = self.cleanliness_classifier(sentence_representation)
